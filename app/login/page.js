...
// Mensajes para los códigos de error con los que NextAuth redirige a /login?error=
const ERROR_MESSAGES = {
  AccessDenied:          "Tu cuenta no está autorizada para acceder al dashboard. Pedile acceso al administrador de Blo S.A.",
  OAuthAccountNotLinked: "Tu email ya está asociado a otro método de acceso. Probá entrando con el otro botón.",
  Configuration:         "Hay un problema de configuración en el acceso. Avisale al administrador.",
  Verification:          "El enlace de verificación expiró o ya fue usado. Volvé a intentarlo.",
};
...
function LoginForm() {
  ...
  const errorMsg = errorParam
    ? (ERROR_MESSAGES[errorParam] ?? "No pudimos completar el inicio de sesión. Volvé a intentarlo.")
    : null;

  function entrarCon(provider) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-2xl p-10 w-full max-w-sm shadow-xl border border-slate-700">
        ...
        {errorMsg && (
          <div role="alert"
               className="mb-6 flex items-start gap-2.5 bg-rose-950/40 border border-rose-900/60
                          text-rose-300 text-sm rounded-xl px-4 py-3 leading-relaxed">
        ...
        <button
          ...
          disabled={!!conectando}
          className="w-full bg-white hover:bg-gray-100 text-gray-900 font-semibold py-3 px-4 rounded-xl transition flex items-center justify-center space-x-3 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          ...
          <span>
            {conectando === "google"
              ? <><i className="ti ti-loader-2 animate-spin mr-1" aria-hidden="true"/>Conectando…</>
        ...
        <button
          ...
          disabled={!!conectando}
          className="w-full mt-3 bg-slate-700/60 hover:bg-slate-700 text-slate-300 text-sm font-medium py-2.5 px-4 rounded-xl transition flex items-center justify-center space-x-2 disabled:opacity-60 disabled:cursor-not-allowed"
        >
