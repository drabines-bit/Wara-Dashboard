...
// CAMBIO 5 — facturacionNeta(t) = MRR(t) + facturacionProyectos(t).
export async function facturacionNeta(mesCerrado = mesCerradoActual(), cantidadMeses = 18) {
  ...
  const unidades = await proyectarUnidades(U_base, mesCerrado, cantidadMeses);
  ...
  for (const fila of tarifa) {
    ...
    resultado.push({ ...fila, proyectos, facturacionNeta: fila.MRR + proyectos });
  }
  return resultado;
}
