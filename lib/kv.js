...
export function getDefaultConfig() {
  return {
    ...
    links: {
      ...
      custom1:    { label: 'Enlace personalizado 1', url: '', enabled: false },
      custom2:    { label: 'Enlace personalizado 2', url: '', enabled: false },
    },
    autoSync: {
      enabled:       false,
      intervalHours: 6,
...
const AUTO_SYNC_KEY = 'wara:autoSyncStatus';

export async function getAutoSyncStatus() {
  try {
    const r = getRedis();
    if (!r) return null;
    return (await r.get(AUTO_SYNC_KEY)) ?? null;
  } catch { return null; }
}

export async function setAutoSyncStatus(data) {
  try {
    const r = getRedis();
    if (!r) return;
    await r.set(AUTO_SYNC_KEY, data);
  } catch {}
}