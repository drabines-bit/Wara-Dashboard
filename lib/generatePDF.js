...
function pageHeader(doc, sub, logoPng) {
...
export async function generateMonthlyReport({ companyData, config, selectedMonthIdx, chartRefs, odooData = {} }) {
  ...
  if (sections.includes('pnl'))
    addPnlPage(doc, pnlData);

  if (sections.includes('odoo_comercial'))
    addOdooComercialPage(doc, comercialData);
