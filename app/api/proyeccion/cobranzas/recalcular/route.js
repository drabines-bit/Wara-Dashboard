...
import { derivarCurva } from "@/lib/proyeccion/cobranzas";

const CRON_MENSUAL = "0 6 1 * *"; // día 1 de cada mes, 06:00
...
// POST — invocado por el cron de QStash (o manualmente con el secreto) para recalcular y persistir la curva.
export async function POST(req) {
  if (!checkCronSecret(req)) {
    return NextResponse.json({ error: "No autorizado" }, { status: 401 });
  }
  try {
    const curva = await derivarCurva();
    return NextResponse.json({ ok: true, curva });
  } catch (err) {
...
export async function PUT(req) {
  ...
  const schedule = await qClient.schedules.create({
    destination: `${appUrl}/api/proyeccion/cobranzas/recalcular`,
    cron: CRON_MENSUAL,
    method: "POST",
    headers: {
