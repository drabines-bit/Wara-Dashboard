...
// CAMBIO 1 — Orquestador: combina ventas + cobranzas + costos en una sola
// grilla de 18 meses con dos lentes (devengado y caja). IVA/Ganancias no se
// estiman: "impuestos" queda expuesto en null a propósito.
export async function GET() {
  ...
  try {
    ...
    const facturacionPorMes = new Map(ventas.map((f) => [f.mes, f.facturacionNeta]));
    const cobranzaPorMes = new Map(cobranzas.detalle.map((c) => [c.mes, c]));

    const detalle = costos.map((c) => {
      const cob = cobranzaPorMes.get(c.mes);
      const facturacionNetaMes = facturacionPorMes.get(c.mes) ?? c.facturacionNeta;
    ...
    // Subtotales anuales: un bloque por cada año calendario presente en la
    // ventana de 18 meses, cerrando (entre otros) en cada 31-dic.
    ...
    return NextResponse.json(
      { detalle, subtotalesAnuales, curvaCobranza: cobranzas.curva },
