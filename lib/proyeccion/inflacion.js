...
// valores = { "YYYY-MM": pct, ... } (pct como número de porcentaje, 2.9 = 2,9%).
// Usado por sync-sheets para importar la fila "Inflación del Mes" de la planilla.
export async function importarRemDesdeSheet(valores) {
  if (!valores || Object.keys(valores).length === 0) return;
  const r = getRedis();
  if (!r) throw new Error("Redis no configurado — verificá las variables de entorno Upstash");
  await r.hset(REM_KEY, valores);
  cache.rem = null; // forzar relectura en la próxima consulta
}
