...
function SectionRow({ label, amount, expanded, onToggle, colorClass = '' }) {
  return (
    <tr className="bg-slate-50 dark:bg-slate-700/40 cursor-pointer select-none"
        onClick={onToggle}>
      <td className="py-2 pl-3 text-xs font-semibold uppercase tracking-wide
  ...
  if (error)
    return (
      <div className="bg-red-50 dark:bg-red-950/30 border border-red-200
                      dark:border-red-800 rounded-2xl p-5 mb-4
                      flex items-start gap-3">
        <i className="ti ti-alert-circle text-red-500 text-xl flex-shrink-0"
           aria-hidden="true"/>
        <div>
    ...
    <div className="mb-6">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 text-xs bg-violet-50
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        {[
          { label: 'Ingresos YTD',      value: resumen.ingresos,         sub: '',                                       color: '' },
          { label: 'Resultado bruto',   value: resumen.resultadoBruto,   sub: `Margen ${resumen.margenBruto.toFixed(1)}%`,  color: resumen.resultadoBruto  >= 0 ? 'pos' : 'neg' },
          { label: 'Gastos operativos', value: resumen.gastosOperativos, sub: resumen.ingresos > 0 ? `${((resumen.gastosOperativos / resumen.ingresos) * 100).toFixed(1)}% s/ ingresos` : '', color: '' },
          { label: 'Resultado neto',    value: resumen.resultadoNeto,    sub: `Margen ${resumen.margenNeto.toFixed(1)}%`,   color: resumen.resultadoNeto   >= 0 ? 'pos' : 'neg' },
        ].map((kpi, i) => (
